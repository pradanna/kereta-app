...
@section('content')
    ...
    @if (\Illuminate\Support\Facades\Session::has('success'))
        <script>
            Swal.fire({
                ...
                window.location.href = '{{ route('facility-certification-train-electric') }}';
            })
        </script>
    @endif
    <div class="d-flex justify-content-between align-items-end mb-4">
        <div class="page-title-container">
            ...
            <p class="mb-0">Manajemen Edit Data Sertifikasi Sarana Kereta Rel Listrik (KRL)</p>
        </div>
                        ...
                        <div class="form-group w-100">
                            ...
                            <select class="select2 form-control" name="train_type" id="train_type"
                                ...
                                @foreach ($train_types as $train_type)
                                    <option value="{{ $train_type->id }}" {{ ($train_type->id === $data->train_type_id) ? 'selected' :'' }}>{{ $train_type->code }} ({{ $train_type->name }})</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
