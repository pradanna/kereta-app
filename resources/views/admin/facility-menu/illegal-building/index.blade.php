...
@section('content')
    <div class="d-flex justify-content-between align-items-end mb-4">
        <div class="page-title-container">
            ...
            <p class="mb-0">Manajemen Data Bangunan Liar {{ $service_unit->name }}</p>
        </div>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0">
                <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
                <li class="breadcrumb-item active" aria-current="page">Bangunan Liar {{ $service_unit->name }}</li>
            </ol>
        </nav>
    </div>
...
@section('js')
    ...
    <script>
        ...
        function getDataIllegalBuildingMap() {
            let area = $('#area-option').val();
            // let name = $('#name').val();
            let url = path + '?type=map&area=' + area;
            return $.get(url)
        }

        function generateMapIllegalBuilding() {
            getDataIllegalBuildingMap().then((response) => {
                removeMultiMarker();
                let data = response.data;
                if (data.length > 0) {
                    createMultiMarkerIllegalBuilding(data)
                }
            }).catch((e) => {
